...
- Often used in combination with supervised approaches to improve the performance of predictive models
    - topic modeling --> identify topics in a collection of documents, which can then be used as labels in a supervised learning model
    - word embeddings --> represent words as dense vectors, which can then be used as input to a supervised learning model
...
- Allows the inclusion of covariates in the model, which can help to explain the relationship between topics and document metadata

- If you have metadata critical to your research questions (understanding how topics vary across groups, over time, or in relation to specific attributes)
...
## @gonzatti2025far
![](images/04_gonzatti_völker.png)


## @gonzatti2025far
![](images/04_gonzatti_völker_2.png)
...
![Difference between sparse and dense vectors (matrices)](images/04_we_sparse-matrix.png)
...
- **Disadvantages**:
    ...
    - As unsupervised as it can get, i.e. what do they mean?
...
- Thus, we can place words and their similarity with the help of vectors: $\text{cosine_similarity}(A, B) = \frac{A \cdot B}{||A|| \cdot ||B||}$
...
- Traditional word embeddings are **static**
    - each word is represented by a single vector that captures the general contexts it typically appears in across the corpus
    - but they do not capture the specific meaning of individual word tokens *in their unique sentence contexts*
...
- Dimensionality and embedding might be hard to understand
