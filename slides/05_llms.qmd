...
- These logits are passed through a **softmax** function:


$\text{softmax}(z_i) = \frac{e^{z_i}}{\sum_j e^{z_j}}$

- This turns the logits into a *probability distribution* over the vocabulary
- The model then:
    - Picks the most likely token (*argmax*)  
    - Or samples based on the probabilities (e.g. *temperature*)

- Softmax ensures that all output probabilities are positive and sum to 1     — key step for **generative models** (aka Decoder models trained with causal language modeling)
...
- It allows you to run models like Llama 2, Mistral, and others on your own hardware
- It is open source and can be used to run models *locally* --> no third party involved, great for data protection
- Tends to be done through JSON calls in Python but you can use the [rollama package](https://github.com/JBGruber/rollama?tab=readme-ov-file) by [Johannes B. Gruber](https://www.johannesbgruber.eu/) 
...
- GPU dependent (again carbon footprint!)
- Data privacy:
