...
- Text unitization : documents, paragraphs, sentences ? 
...
- Most of text classification projects rely on RAs or crowdworkers
    ...
    - Quality of annotations checked with inter-coder reliability (ICR) tests
- As phd students, we often do it ourselves
    - Time consuming, but we know the data better
    - But if you want to publish, you need to show ICR
- More and more use of LLms to annotate texts : highly discussed in the literature
...
- Split the annotated sample into a training set and a test set (70/30 rule)
- Train a model on the training set that learns the relationship between the text features and the labels
...
- Often used as to measure previously unmeasured concepts
- Often some aggregated measure of the categories is used
- Descriptives : variation over time, across countries, groups
- Downstream tasks : 
...
## What to do with classification outputs ? {.smaller}

![@licht2024measuring : use of classification output as DV](images/03_licht_populism.png)
