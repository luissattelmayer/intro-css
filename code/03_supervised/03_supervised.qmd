...
```{r}
# Packages need to be installed beforehand using install.packages("package_name")
...
```{r}
df <- read_csv("https://cta-text-datasets.s3.eu-central-1.amazonaws.com/labeled/benoit_crowdsourced_2016/benoit_crowdsourced_2016-policy_area.csv")
...
economy_dfm <- economy_recipe |> 
  prep() |> 
  bake(new_data = NULL) |> 
bind_cols(df_train |> select(text))

economy_dfm |> select(text, label, tf_text_tax, tf_text_financet) |> view()
...
preds_economy_rf |> 
    slice_sample(n =5) |> 
    select(text, label, .pred_class, .pred_1) |> view()
```
...
# Set 1 to be main class

preds_economy_rf <- preds_economy_rf |> mutate(
    label = fct_relevel(label, c("1", "0")),
    .pred_class = fct_relevel(.pred_class, c("1", "0"))
)
...
preds_economy_rf <- preds_economy_rf |> 
    mutate(
        pred_keywords = case_when(
            ...
            .default = 0
        ) |> as.factor() |> fct_relevel(c("1", "0")) # Relevel to match the label factor
    )
...
preds_economy_rf |> 
    filter(pred_keywords == 1) |> 
    select(pred_keywords, label, text) |> view()
...
preds_manifesto |> 
    ...
    # Compute counts of economy and non economy sentences by party
    group_by(partyname) |>
    count(.pred_class) |> 
    # Compute proportions
    mutate(prop =n/sum(n)*100) |> 
    # Keep only economy related sentences
    filter(.pred_class == 1) |> 
...
preds_manifesto |> 
    filter(partyname %in% c("Lab", "Tories", "Lib Dems")) |> 
    ...
    labs(
        x = "Year",
        y = "Proportion of Economy-Related Sentences (%)"
    )
```
