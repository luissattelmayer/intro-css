...
text <- page_html |> 
  html_element(".field--type-text-with-summary .field__item") |>  
  html_text2()
...
In this case, the worflow consists in extracting the urls of all press releases of a page, then moving to the next page and repeating the process until there are no more pages. Here, we have 2050 pages where press releases are stored with around 10 press releases per page (n ~ 20500 press releases).

```{r}
# Create a vector of all of the urls

unsg_page_urls <- str_c("https://press.un.org/en/content/secretary-general/press-release?page=", 0:10)
...
pr <- collect_unsg_pr_urls("https://press.un.org/en/content/secretary-general/press-release?page=5")
pr

# Test sur 10 pages because take around 35-40 minutes to run

unsg_pr_urls <- map(unsg_page_urls, safely(collect_unsg_pr_urls), .progress = T) |> 
    map_df("result") 
...
unsg_df <- map(unsg_pr_urls$link[1:10], safely(collect_unsg_pr_content), .progress = T) |> 
    map_df("result")
