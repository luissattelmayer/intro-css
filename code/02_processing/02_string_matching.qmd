...
str_c(text, "Obama was president in 2016. ", sep = " ") # Concatenate strings
...
```{r}
texts
str_detect(text, "Obama") # Detect the word Trump in the text
...
str_extract(text, "\\d+") # Extract Donald Trump
str_length(text)
...
```{r}
str_view(text, "[a-d]") # Highlight word characters in the text
str_view(text, "[A-Z]") 
...
unsg_palestine <- unsg_df |> 
    filter(str_detect(text, "Palestine|Palestinian|[Gg]aza|[Ww]est [Bb]ank|Hamas"))
...
unsg_df <- unsg_df |>
    mutate(
        palestine_keywords_dummy = str_detect(text, "[Pp]alestine|[Gg]aza|[We]st [Bb]ank|Hamas"),
        palestine_keywords_count = str_count(text, "[Pp]alestine|[Gg]aza|[We]st [Bb]ank|Hamas")
    )

unsg_df |> 
    filter(palestine_keywords_count > 5)
```
