...
```{r}
fasttext <- data.table::fread("/Users/backup-luis/Downloads/cc.en.300.vec.gz", 
                              quote ="", 
                              skip = 1, 
                              data.table = F, 
                              nrows = 5e5)
...
```{r}
semantic_algebra("italy", "germany", "hitler")
```

```{r}
semantic_algebra("germany", "japan", "sushi")
```
...
```{r}
semantic_algebra("paris", "france", "CEE")
```
...
# Terms to analyze
terms <- c("banker", "teacher", "janitor", "entrepreneur", "plumber", "artists", "artist")
...
```{r}
job_query <- c("Economist", "banker" "Sociologist", "Psychologist", "Anthropologist", 
...
```{r}
job_query <- c("lion", "tiger", "duck", "dog", "cat", "python") |> 
  str_to_lower()
