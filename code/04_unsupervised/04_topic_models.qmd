...
In this script [^1], we are going to take a look at topic modeling. More precisely, we are going to implement model based on the Latent Dirichlet Allocation (LDA) and Structural Topic Models (STM). The advantage of STMs is that they can incorporate metadata into the model, which can be used to explain the topics. 

[^1]: This script is inspired by [Felix Lennert](https://felix-lennert.netlify.app/)
...
```{r}
#| eval: false
sotu_lda_k10 <- LDA(sotu_dtm, k = 10, control = list(seed = 1312))
...
```{r}
#| include: false
sotu_lda_k10_tidied <- read_csv("data/sotu_lda_k10_tidied.csv")
```
...
```{r}
#| include: false
determine_k <- read_rds("data/determine_k.rds")
```
...
```{r}
#| include: false
sotu_content_fit <- read_rds("data/sotu_content_fit.rds")
```
